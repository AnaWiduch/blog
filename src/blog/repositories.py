...
# Хранилище данных и сущностей

'''Абстрактные репозитории (ArticlesRepository, UsersRepository) нужны на тот случай,
 если мы захотим перейти от хранения данных в shelve к, например, хранению в PostgreSQL. 
 В таком случае мы просто отнаследуемся от абстрактного репозитория, сделаем новый и 
 подменим его в тех местах, где хотим перейти на другое решение.

Для наглядности мы также не храним нашего админа в БД, 
но код репозитория пользователей можно достаточно просто переписать на другой.

Еще из важного отметим, что пароль у нас не зашифрован. 
Обычно его шифруют односторонними алгоритмами шифрования, такими как sha256 и другими.
 Мы не стали его шифровать, опять же, для наглядности. Если вы захотите его зашифровать, 
 то это можно сделать достаточно быстро.'''